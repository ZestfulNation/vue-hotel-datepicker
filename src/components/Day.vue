<template>
    <div>
        <div class="datepicker__tooltip" v-html="tooltipMessageDisplay" v-if="showTooltip && options.hoveringTooltip" />
        <div
            class="datepicker__month-day"
            @click.prevent.stop="dayClicked($event, date)"
            :class="[dayClass, checkinCheckoutClass, bookingClass, { 'datepicker__month-day--today': isToday }]"
            :tabindex="tabIndex"
            ref="day"
        >
        ...
        notAllowedDayDueToNextPeriod(currentPeriod) {
            ...
            if (nextPeriod) {
                ...
                const resultDate = new Date(
                    subtractDate.setDate(subtractDate.getDate() - currentPeriod.minimumDuration),
                )
        ...
        checkIfDisabled() {
            this.isDisabled =
                ...
                (this.sortedDisabledDates
                    ? this.sortedDisabledDates.some(i => this.compareDay(i, this.date) === 0)
                    : null) ||
